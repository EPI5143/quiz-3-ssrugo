...
# (.R which is simply a plain text file) and submit it to Github using instructions 
...
ncol(mpg) # 11 columns (vars)
dim(mpg) # 234 rows (observations) and 11 columns (vars)
...
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, color=class, shape=drv, size=cyl)) +
  labs(title="Highway fuel efficiency vs. engine size")
